...
`github.repository_owner`, **lowercased** in the tags step via bash parameter
expansion (`${GITHUB_REPOSITORY_OWNER,,}` — GitHub Actions expressions have no
`lower()` function) — Docker image references are lowercase-only, so the
workflow lowercases the owner and `.env`/compose use `ghcr.io/reckz69` (GHCR
normalizes case, so `ghcr.io/Reckz69/...` and `ghcr.io/reckz69/...` are the
same artifact) — see ADR-016.
